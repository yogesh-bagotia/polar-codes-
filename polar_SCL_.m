...
Q=[ 1 2 4 8 16 32 3 5 64 9 6 17 10 18 128 12 33 65 20 256 34 24 36 7 129 66 512 11 40 68 130 ...
...
Nblocks = 10000; 
BER_sim = zeros(size(EbNodB_range));
BLER_sim = zeros(size(EbNodB_range));
...
%Simulate
for idx = 1:length(EbNodB_range)
        EbNodB = EbNodB_range(idx);
        EbNo = 10^(EbNodB / 10);
    ...
    parfor blk = 1:Nblocks
        ...
        %Counting errors
        bit_errors = sum(msg ~= msg_cap);
        Nbiterrs = Nbiterrs + bit_errors;
    
        if bit_errors > 0
            Nblockerrs = Nblockerrs + 1;
        end
    end
    BER_sim(idx) = Nbiterrs / (K * Nblocks);
    BLER_sim(idx) = Nblockerrs / Nblocks;
    disp([EbNodB BER_sim(idx)])
end


figure;
semilogy(EbNodB_range, BER_sim, 'o-', 'LineWidth', 2);
grid on;
xlabel('Eb/N0 (dB)');
ylabel('Bit Error Rate (BER)');
title(sprintf('Polar Code SCL Decoder: N = %d, K = %d, Rate = %.2f', N, K, Rate));

% --- Plot: Probability of Successful Decoding ---
P_success = 1 - BLER_sim;
figure;
semilogy(EbNodB_range, P_success, 's-', 'LineWidth', 2);
grid on;
xlabel('Eb/N0 (dB)');
ylabel('Probability of Successful Decoding');
title(sprintf('Success Probability (Block Level): N = %d, K = %d, Rate = %.2f', N, K, Rate));
